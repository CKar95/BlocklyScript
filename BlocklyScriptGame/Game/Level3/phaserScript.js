			...
			if(blocklyScriptCompleted && !moving) {
				...
				if(starCollected === 13 && runOnce) {
					...
					if(totalBlocksUsed === 12){
						...
						levelBadge = 1;
						outputArea.value +=  '\n Congratulations. You used the minimum number of blocks in order to complete Level 3. You are rewarded with +100 points, added to your current Score! Your total score is: '+((blocklyPoints + totalScore + levelScore) - Number(losePoints())) + "";
						...
						outputArea.value +=  '\n';
						outputArea.value +=  '\n Well done. You completed Level 3. However you used more blocks than Level 3 needed so you are rewarded with +40 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
						
					}else if(totalBlocksUsed < 12){
						blocklyPoints = 150;
						document.getElementById("levelbadge").src="../assets/first.png";
						levelBadge = 1;
						outputArea.value +=  '\n Wow you are a genius. You completed Level 3 with less blocks than needed. You are rewarded with +150 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
					...
					runOnce = false;
					outputArea.value +=  '\n';
					outputArea.value +=  'Tip: If you used two or more blocks of the same type under each other, then you should place that type of block 1 time inside a repeat N times block';
					outputArea.value +=  '\n';

					gameEndedText.setText('Try again.\n Collect every star.');
					
					if(useLaserBeam === 0 || shootBeamDir=== 0){
						outputArea.value +=  '\n In order to clear the path from the alien you have to use your laser beam and aim it at the right direction';
					}else if(repeatNTimes < 3){
						outputArea.value +=  '\n You may stacked blocks of the same type under each other. Use that type of block 1 time inside repeat N times block.';
					}else if(jumpBlock < 3 ){
						outputArea.value +=  '\n You have to use the jump block at least 3 times in order to win this Level';
					}else{
						outputArea.value +=  '\n Hmm it seems that you didn\'t make it. Try again you are almost there';
					}
					outputArea.value +=  '\n';
					outputArea.value +=  '\n';
