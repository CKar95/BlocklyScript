			...
			if(blocklyScriptCompleted && !moving) {
				...
				if(starCollected === 20 && runOnce) {
					...
					if(totalBlocksUsed === 31){
						...
						levelBadge = 1;
						outputArea.value +=  '\n Congratulations. You used the minimum number of blocks in order to complete Level 9. You are rewarded with +100 points, added to your current Score! Your total score is: '+((blocklyPoints + totalScore + levelScore) - Number(losePoints())) + "";
						...
						outputArea.value +=  '\n';
						outputArea.value +=  '\n Well done. You completed Level 9. However you used more blocks than Level 9 needed so you are rewarded with +40 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
						
					}else if(totalBlocksUsed < 31){
						blocklyPoints = 40;
						document.getElementById("levelbadge").src="../assets/second.png";
						levelBadge = 2;
						outputArea.value +=  '\n Well done. You completed Level 9, but you probably didn\'t use the "if"(condition) block the right way. You should try to use them in order to get more points. You are rewarded with +40, added to your current Score."' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
					...
					runOnce = false;
					outputArea.value +=  '\n';
					outputArea.value +=  'Tip:  Create a method and give it a name you like. The method should not be connected with the Start block. Inside the method add blocks that will help you check the color of the star the adventurer step on, collect balloons and stars. First thing you have to check is the color of the star with an "if" block. Then inside that "if" add a second (nested)"if" block that will check if the star has a balloon on top of it. If it has a balloon place a " jump and collect option balloon" block inside your nested if. After that and outside your nested "if" block use "collect option star" block. Do this for all the colors of the stars. When you are done with your method, call it under your start block at the right position ';
					outputArea.value +=  '\n';
					outputArea.value +=  '\n';
					...
					for( var i=0; i < blocks.length; i++) {
						if( i === 0){
							...
							if(blocks[i].type !== "repeat_until") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "repeat untill wall is option" block. Check for the right options \n';
								break;
							}
						}else if(i === 2) {
							if(blocks[i].type !== "laser_beam") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "use laser beam" block inside your repeat block.\n';
								break;
							}
						}else if(i === 3) {
							if(blocks[i].type !== "laser_beamdirection") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "shoot beam option" block here. \n';
								break;
							}
						}else if(i === 4) {
							if(blocks[i].type !== "laser_beam") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "use laser beam" block to attack the alien \n';
								break;
							}
						}else if(i === 5) {
							if(blocks[i].type !== "laser_beamdirection") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "shoot beam option" block.\n';
								break;
							}
						}else if(i === 6) {
							...
							if(blocks[i].type !== "procedures_callnoreturn") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to call the method you created here. Check that tha name of the call method block is the same as the method you created\n';
								break;
							}
						}			
