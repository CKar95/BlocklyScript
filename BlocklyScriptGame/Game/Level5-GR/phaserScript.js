			...
			if(blocklyScriptCompleted && !moving) {
				...
				if(starCollected === 19 && runOnce) {
					...
					if(totalBlocksUsed === 21){
						...
						levelBadge = 1;
						outputArea.value +=  '\n Συγχαρητήρια. Χρησιμοποίησες τον ελάχιστο αριθμό μπλοκ για να κερδίσεις το Επίπεδο 5. Θα ανταμειφτείς με +100 πόντους, οι οποίοι θα προστεθούν στο τρέχον σκορ σου! Το συνολικό σου σκορ είναι: '+((blocklyPoints + totalScore + levelScore) - Number(losePoints())) + "";
						...
						outputArea.value +=  '\n';
						outputArea.value +=  '\n Καλή προσπάθεια. Ολοκλήρωσες το Επίπεδο 5. Ωστόσο χρησιμοποίησες περισσότερα μπλοκ για να κερδίσεις το Επίπεδο 5 από ότι χρειαζόταν και για αυτόν το λόγο θα ανταμειφτείς με +40 πόντους, οι οποίοι θα προστεθούν στο τρέχον σκορ σου! Το συνολικό σου σκορ είναι: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
						
					}else if(totalBlocksUsed < 21){
						blocklyPoints = 150;
						document.getElementById("levelbadge").src="../assets/first.png";
						levelBadge = 1;
						outputArea.value +=  '\n Συγχαρητήρια. Ολοκλήρωσες το Επίπεδο 5 χρησιμοποιώντας λιγότερα μπλοκ από ότι χρειαζόταν. Θα ανταμειφτείς με +150 πόντους, οι οποίοι θα προστεθούν στο τρέχον σκορ! Το συνολικό σου σκορ είναι: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
					...
					runOnce = false;
					outputArea.value +=  '\n';
					outputArea.value +=  'Συμβουλή: Το "Repeat until wall is" μπλοκ θα επαναλαμβάνει όλα τα μπλοκ εντολών που τοποθέτησες μέσα του εάν η συνθήκη (broken or not broken) είναι ψευδής.';
					outputArea.value +=  '\n';
					...
					for( var i=0; i < blocks.length; i++) {
						if( i === 0){
							...
							if(blocks[i].type !== "repeat_until") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το "repeat until wall is" μπλοκ με την σωστή συνθήκη. Εάν δεν χρησιμοποίησες αυτό το μπλοκ εδώ τότε θα χρειαστεί αργότερα να χρησιμοποίησες περισσότερα μπλοκ. \n';
								break;
							}
						}else if(i === 2) {
							if(blocks[i].type !== "laser_beam") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το "use laser beam" μπλοκ μέσα στο repeat while μπλοκ. \n';
								break;
							}
						}else if(i === 3) {
							if(blocks[i].type !== "laser_beamdirection") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Έδωσες κατεύθυνση για την ακτίνα φωτός;. \n';
								break;
							}
						}else if(i === 4) {
							if(blocks[i].type !== "controls_repeat") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το repeat N times μπλοκ εδώ. Αυτό το μπλοκ θα πρέπει να τοποθετηθεί έξω από το reapeat while μπλοκ \n';
								break;
							}
						}else if(i === 5) {
							if(blocks[i].type !== "move_right") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το move right μπλοκ εδώ. Αυτό το μπλοκ θα πρέπρει να βρίσκεται μέσα στο reapeat N times μπλοκ\n';
								break;
							}
						}else if(i === 6) {
							if(blocks[i].type !== "controls_repeat") {
							...
							if(blocks[i].type !== "move_left") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το move left μπλοκ έξω από το repeat N times μπλοκ.\n';
								break;
							}
						}else if(i === 8) {
							if(blocks[i].type !== "move_up") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το jump block εδώ.\n';
								break;
							}
						}
						else if(i === 9) {
							...
							if(blocks[i].type !== "move_right") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το move right μπλοκ μέσα στο repeat N times μπλοκ.\n';
								break;
							}
						}else if(i === 11) {
							if(blocks[i].type !== "move_up") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το jump μπλοκ έξω από το repeat μπλοκ.\n';
								break;
							}
						}else if(i === 12) {
							if(blocks[i].type !== "move_left") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το move left μπλοκ.\n';
								break;
							}
						}else if(i === 13) {
							if(blocks[i].type !== "controls_repeat") {
							...
							if(blocks[i].type !== "laser_beamdirection") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το shoot beam direction μπλοκ εδώ.\n';
								break;
							}
						}else if(i === 16) {
							if(blocks[i].type !== "controls_repeat") {
							...
							if(blocks[i].type !== "move_left") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Χρησιμοποίησε το move left μπλοκ μέσα στο repeat μπλοκ.\n';
								break;
							}
						}
					...
					if(blocks.length < 21){
						outputArea.value +=  '\nΑπό ότι φαίνεται δεν τα κατάφερες. Τοποθέτησε περισσότερα μπλοκ για να κερδίσεις το επίπεδο.\n';
					}
