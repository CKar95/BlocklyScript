			...
			if(blocklyScriptCompleted && !moving) {
				...
				if(starCollected === 14 && runOnce) {
					...
					if(totalBlocksUsed === 25){
						...
						levelBadge = 1;
						outputArea.value +=  '\n Congratulations. You used the minimum number of blocks in order to complete Level 7. You are rewarded with +100 points, added to your current Score! Your total score is: '+((blocklyPoints + totalScore + levelScore) - Number(losePoints())) + "";
						...
						outputArea.value +=  '\n';
						outputArea.value +=  '\n Well done. You completed Level 7. However you used more blocks than Level 7 needed so you are rewarded with +40 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
						
					}else if(totalBlocksUsed < 25){
						blocklyPoints = 40;
						document.getElementById("levelbadge").src="../assets/second.png";
						levelBadge = 2;
						outputArea.value +=  '\n Well done. You completed Level 7, but you probable didn\'t use the "if"(condition) block. You should try to use them in order to get more points. You are rewarded with +40, added to your current Score."' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
					...
					runOnce = false;
					outputArea.value +=  '\n';
					outputArea.value +=  'Tip:  If (condition) block will execute all the blocks you put inside it once, if your condition is true. If your condition is false  then it will skip all the blocks you put inside it. You can also expand your if block and drag and drop an else if and an else under your first if. That way you can check if the adventurer steps on a red, green or blue star.';
					outputArea.value +=  '\n';
					outputArea.value +=  '\n';
					...
					for( var i=0; i < blocks.length; i++) {
						if( i === 0){
							...
							if(blocks[i].type !== "laser_beam") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "use laser beam" block. \n';
								break;
							}
						}else if(i === 2) {
							if(blocks[i].type !== "laser_beamdirection") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "shoot beam direction" block..\n';
								break;
							}
						}else if(i === 3) {
							if(blocks[i].type !== "controls_repeat") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "control N times" block here. \n';
								break;
							}
						}else if(i === 4) {
							if(blocks[i].type !== "move_right") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "move right" block inside repeat block \n';
								break;
							}
						}else if(i === 5) {
							if(blocks[i].type !== "controls_if") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "if" block here. You will have to expand it and use one "else if" and one "else" in order to check all the colors.\n';
								break;
							}
						}else if(i === 6) {
							...
							if(blocks[i].type !== "collect_star") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a collect option star block inside the if block.\n';
								break;
							}
						}else if(i === 8) {
							if(blocks[i].type !== "star_color") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "star has option color" block next to the else if. If you don\'t have an else if the expand your if block and drag and drop and else if option.\n';
								break;
							}
						}
						else if(i === 9) {
							...
							if(blocks[i].type !== "collect_star") {
								outputArea.value +=  '\nBlock '+(i + 1)+': If you have 2 else if inside your if block then you should remove your last else if and instead replace it with an else. That way you wont have to use an extra star has option color block. Inside the else use your collect option star. \n';
								break;
							}
						}
