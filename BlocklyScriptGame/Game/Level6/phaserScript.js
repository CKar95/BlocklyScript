			...
			if(blocklyScriptCompleted && !moving) {
				...
				if(starCollected === 15 && runOnce) {
					...
					if(totalBlocksUsed === 15){
						...
						levelBadge = 1;
						outputArea.value +=  '\n Congratulations. You used the minimum number of blocks in order to complete Level 6. You are rewarded with +100 points, added to your current Score! Your total score is: '+((blocklyPoints + totalScore + levelScore) - Number(losePoints())) + "";
						...
						outputArea.value +=  '\n';
						outputArea.value +=  '\n Well done. You completed Level 6. However you used more blocks than Level 6 needed so you are rewarded with +40 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
						
					}else if(totalBlocksUsed < 15){
						blocklyPoints = 40;
						document.getElementById("levelbadge").src="../assets/second.png";
						levelBadge = 2;
						outputArea.value +=  '\n Well done. You completed Level 6, but you probably didn\'t use the "if"(condition) block. You should try to use them in order to get more points. You are rewarded with +40, added to your current Score."' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
					...
					runOnce = false;
					outputArea.value +=  '\n';
					outputArea.value +=  'Tip:  If (condition) block will execute all the blocks you put inside it once, if your condition is true. If your condition is false  then it will skip all the blocks you put inside it.';
					outputArea.value +=  '\n';
					outputArea.value +=  '\n';
					...
					for( var i=0; i < blocks.length; i++) {
						if( i === 0){
							...
							if(blocks[i].type !== "move_left") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "move left" block inside a repeat N times block. First you wann move the adventurer and then check the color of the star.\n';
								break;
							}
						}else if(i === 3) {
							if(blocks[i].type !== "controls_if") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "if" block here. \n';
								break;
							}
						}else if(i === 4) {
							if(blocks[i].type !== "star_color") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "star has: \'option\' color" here. Check if the adventurer is right behind of the color of your selected option \n';
								break;
							}
						}else if(i === 5) {
							if(blocks[i].type !== "collect_star") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a "collect \'option\' star block here. Select the right color.\n';
								break;
							}
						}else if(i === 6) {
							if(blocks[i].type !== "controls_repeat") {
							...
							if(blocks[i].type !== "move_up") {
								outputArea.value +=  '\nBlock '+(i + 1)+': Try to use a jump block inside the repeat N times block.\n';
								break;
							}
						}else if(i === 8) {
