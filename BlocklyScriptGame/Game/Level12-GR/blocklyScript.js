        ...
        interpreter.createNativeFunction(wrapper));
		 ...
		 var wrapper = function() {
		  ...
		  interpreter.setProperty(scope, 'checkStarOverlap',
			...
			return restoreStarsFun();
		  };
		  interpreter.setProperty(scope, 'restoreStars',
			  interpreter.createNativeFunction(wrapper));
      ...
      if (!myInterpreter) {
		...
		for( var i=0; i < blocks.length; i++) {
			if(blocks[i].type === "move_right") {
				...
				openDoorWithPass++;
			}else if (blocks[i].type === "restore_stars") {
				resetStarsBlock++;
			}else {
			}
		}
		
		totalBlocksUsed = (moveLeft + startBlock + moveRight + jumpBlock) + (useLaserBeam + shootBeamDir + repeatNTimes) + repeatWhile + repeatUntil + (ifBlocks + starHasColor+ collectColor) + (starHasBalloon + collectBalloon) + (method + callMethod) + (setPassword + openDoorWithPass) + resetStarsBlock;
		...
		blocksUsedText += "Restore Stars μπλοκ που χρησιμοποίηθηκαν: " + resetStarsBlock + "\n";
