  ...
  var wrapper = interpreter.createNativeFunction(
    function(x, callback) {
		if(x === 1 && starColorOverlap === 'red'){
			...
			return true;
		}else if(x === 2 && starColorOverlap === 'green'){
			console.log("trueeeeeee");
			return true;
		}else if(x === 3 && starColorOverlap === 'blue'){
			console.log("trueeeeeee");
			return true;
		}else {
    ...
    function(x, callback) {
		setTimeout(function(){ 
              if(x === 1){
				...
				collectColorfulStar = 'green';
				console.log("Color:" + collectColorfulStar);
			}else if(x === 3){
				collectColorfulStar = 'blue';
				console.log("Color:" + collectColorfulStar);
			}
            callback(); },  1000);
    });
...
Blockly.JavaScript['star_hasballon'] = function(block) {
  // TODO: Assemble JavaScript into code variable.
  var code = 'checkStarHasBalloon()';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};


function initInterpreterCheckStarHasBalloon(interpreter, scope) {
  // Ensure function name does not conflict with variable names.
  // uses to time outs, terminates the upward motion before termination moving right
  Blockly.JavaScript.addReservedWords('checkStarHasBalloon');
  var wrapper = interpreter.createNativeFunction(
    function() {
		return starHasBalloon;
    });
  interpreter.setProperty(scope, 'checkStarHasBalloon', wrapper);
}

Blockly.JavaScript['collect_balloon'] = function(block) {
  // TODO: Assemble JavaScript into code variable.
  var colour_starcolor = block.getFieldValue('COLOUR');
  var code = '\n';
  ...
  if( String(colour_starcolor) === '#ff0000')
   {
	   code = "collectBalloon("+1+");\n";
   }else if(String(colour_starcolor) === '#008000') {
	   code = "collectBalloon("+2+");\n";
   }else {
	   code = "collectBalloon("+3+");\n";
   }
  return code;
  return code;
};
...
function initInterpreterCollectBalloon(interpreter, scope) {
  // Ensure function name does not conflict with variable names.
  // uses to time outs, terminates the upward motion before termination moving right
  Blockly.JavaScript.addReservedWords('collectBalloon');
  var wrapper = interpreter.createAsyncFunction(
    function(x, callback) {
      goJump = true;
	  
			if(x === 1){
				collectColorfulBalloon = 'red';
				console.log("Color:" + collectColorfulBalloon);
			}else if(x === 2){
				collectColorfulBalloon = 'green';
				console.log("Color:" + collectColorfulBalloon);
			}else if(x === 3){
				collectColorfulBalloon = 'blue';
				console.log("Color:" + collectColorfulBalloon);
			}
			setTimeout(function () {
					goJump = false;
          ...
          setTimeout(function(){ 
			...
			collectColorfulBalloon = '';			
            callback(); },  1400);


    });
  interpreter.setProperty(scope, 'collectBalloon', wrapper);
}


Blockly.JavaScript['open_door'] = function(block) {
  var variable_getpass = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('getPass'), Blockly.Variables.NAME_TYPE);
  // TODO: Assemble JavaScript into code variable.
  //console.log(demoWorkspace.getVariable(value_opendoor));
  console.log("the pass is " + variable_getpass);
  var code = 'openDoor('+variable_getpass +');\n';
  return code;
};


function initInterpreterOpenDoor(interpreter, scope) {
  // Ensure function name does not conflict with variable names.
  // uses to time outs, terminates the upward motion before termination moving right
  Blockly.JavaScript.addReservedWords('openDoor');
  var wrapper = interpreter.createAsyncFunction(
    function(x, callback) {
      
	  console.log("the pass is: "+ x);
  ...
  interpreter.setProperty(scope, 'openDoor', wrapper);
}

Blockly.JavaScript['restore_stars'] = function(block) {
  // TODO: Assemble JavaScript into code variable.
  var code = 'restoreStars();\n';
  return code;
};
