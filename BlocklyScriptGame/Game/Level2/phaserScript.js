			...
			if(blocklyScriptCompleted && !moving) {
				...
				if(starCollected === 7 && runOnce) {
					...
					if(totalBlocksUsed === 15){
						...
						levelBadge = 1;
						outputArea.value +=  '\n Congratulations. You used the minimum number of blocks in order to complete Level 2. You are rewarded with +50 points, added to your current Score! Your total score is: '+((blocklyPoints + totalScore + levelScore) - Number(losePoints())) + "";
						
					}else if(totalBlocksUsed > 15){
						blocklyPoints = 30;
						document.getElementById("levelbadge").src="../assets/second.png";
						levelBadge = 2;
						outputArea.value +=  '\n Well done. You completed Level2. However you used more blocks than Level2 needed so you are rewarded with +30 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
						blocklyPoints = 30;
					}else if(totalBlocksUsed < 15){
						blocklyPoints = 100;
						document.getElementById("levelbadge").src="../assets/first.png";
						levelBadge = 1;
						outputArea.value +=  '\n Wow you are a genius. You completed Level2 with less blocks than needed. You are rewarded with +100 points, added to your current Score! Your total score is: ' +((blocklyPoints + totalScore + levelScore)  - Number(losePoints()))+ "";
					...
					runOnce = false;
					outputArea.value += '\nRemember to use the Jumb block before you try to Move Left or Right';
					outputArea.value += '\nDon\'t stack two or more jump blocks under each other. The adventurer will jump only once very fast and then continue executing the rest of the commands.';
					outputArea.value += '\nTip: Try to let the adventurer fall from the 1st floor ground and then use the jump block.';
					gameEndedText.setText('Try again.\n Collect every star.');
					
					if(moveRight < 3 || moveLeft < 3){
						outputArea.value +=  '\n You have to add at least 3 blocks Move Left and Move Right. Try to add more Move blocks';
					}else if(jumpBlock < 5){
						outputArea.value +=  '\n You have to add at least 5 jump blocks. Try to add more jump blocks';
					}else{
